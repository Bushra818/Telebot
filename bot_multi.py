...
from telegram import BotCommand, BotCommandScopeChat, InlineKeyboardButton, InlineKeyboardMarkup, InputFile, Update, WebAppInfo
...
WEB_PUBLIC_BASE_URL = os.getenv(
    "WEB_PUBLIC_BASE_URL",
    "https://telebot-qk9v.onrender.com",
).rstrip("/")
...
try:
    ...
    FastAPI = None
    HTTPException = Query = None
    HTMLResponse = StreamingResponse = None
    BaseModel = object
    uvicorn = None
    logger.warning("FastAPI/Uvicorn غير مثبتين؛ Mini App لن تعمل حتى يتم تثبيتهما.")
...
<style>
*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,sans-serif;background:#10151d;color:#e8edf4}.app{display:flex;height:100vh}.side{width:330px;border-left:1px solid #29313d;background:#151b24;display:flex;flex-direction:column}.main{flex:1;display:flex;flex-direction:column;min-width:0}.top{height:62px;border-bottom:1px solid #29313d;display:flex;align-items:center;padding:0 18px;gap:10px}h2,h3{margin:0}.search{margin:12px}.search input,.composer input{width:100%;background:#0d1219;border:1px solid #303b49;color:#fff;border-radius:10px;padding:11px}.sessions,.dialogs{overflow:auto;flex:1}.item{padding:12px 14px;border-bottom:1px solid #242c37;cursor:pointer}.item:hover,.item.active{background:#202a37}.badge{font-size:11px;padding:3px 7px;border-radius:8px;background:#26374b}.muted{color:#8e9aaa;font-size:12px}.messages{flex:1;overflow:auto;padding:18px;display:flex;flex-direction:column;gap:10px}.msg{max-width:min(720px,85%);background:#1c2632;border-radius:13px;padding:10px 12px;align-self:flex-start}.msg.out{align-self:flex-end;background:#24435a}.meta{font-size:11px;color:#8fa0b3;margin-top:5px}.media{margin-top:8px}.media img,.media video{max-width:100%;border-radius:10px;max-height:430px}.file{display:inline-block;padding:9px;background:#111820;border-radius:9px;color:#fff;text-decoration:none}.composer{display:flex;gap:8px;padding:12px;border-top:1px solid #29313d}.composer input{flex:1}.btn{border:0;background:#2b83f6;color:white;border-radius:9px;padding:10px 15px;cursor:pointer}.btn.alt{background:#283341}.empty{padding:30px;text-align:center;color:#8996a6}.row{display:flex;gap:8px;align-items:center}.title{font-weight:700}.auth-error{padding:28px;text-align:center}@media(max-width:800px){.side{width:260px}}@media(max-width:600px){.app{display:block}.side{width:100%;height:38vh}.main{height:62vh}.top{height:54px}.msg{max-width:92%}}
</style></head>
<body><div class="app"><aside class="side"><div class="top"><h3>📱 حسابات Telegram</h3></div><div class="search"><input id="sessionSearch" placeholder="بحث في الجلسات..." oninput="filterSessions()"></div><div id="sessions" class="sessions"><div class="empty">جاري التحقق...</div></div></aside><main class="main"><div class="top"><div style="flex:1"><div id="chatTitle" class="title">اختر محادثة</div><div id="chatSub" class="muted"></div></div><button class="btn alt" onclick="loadDialogs()">🔄 تحديث</button></div><div class="search"><input id="chatSearch" placeholder="🔎 بحث في المحادثات أو الرسائل..." onkeydown="if(event.key==='Enter') searchMessages()"></div><div id="dialogs" class="dialogs"></div><div id="messages" class="messages"><div class="empty">اختر محادثة لعرض الرسائل.</div></div><div class="composer"><input id="message" placeholder="اكتب رسالة..." onkeydown="if(event.key==='Enter') sendMessage()"><button class="btn" onclick="sendMessage()">إرسال</button></div></main></div>
<script>
const tg=window.Telegram&&window.Telegram.WebApp?window.Telegram.WebApp:null;const initData=tg?(tg.initData||""):"";let sessions=[],currentSession=null,currentChat=null,sessionFilter="";const $=id=>document.getElementById(id);if(tg){tg.ready();tg.expand();if(tg.setHeaderColor)tg.setHeaderColor('#10151d');if(tg.setBackgroundColor)tg.setBackgroundColor('#10151d')}function authUrl(path){const join=path.includes('?')?'&':'?';return path+join+'init_data='+encodeURIComponent(initData)}async function api(path,opts={}){if(!initData)throw new Error('يجب فتح Mini App من داخل Telegram.');const r=await fetch(authUrl(path),opts);if(!r.ok)throw new Error(await r.text());return r.json()}function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}async function init(){if(!tg||!initData){$('sessions').innerHTML='<div class="auth-error"><h3>⚠️ يجب فتح Mini App من Telegram</h3></div>';return}try{sessions=await api('/api/sessions');renderSessions();if(sessions.length){currentSession=sessions[0].id;renderSessions();await loadDialogs()}else $('dialogs').innerHTML='<div class="empty">لا توجد جلسات صالحة.</div>'}catch(e){$('sessions').innerHTML='<div class="auth-error"><h3>⚠️ تعذر المصادقة</h3><div class="muted">'+esc(e.message)+'</div></div>'}}function renderSessions(){$('sessions').innerHTML='';sessions.filter(s=>(s.name+' '+s.username).toLowerCase().includes(sessionFilter.toLowerCase())).forEach(s=>{const d=document.createElement('div');d.className='item '+(s.id===currentSession?'active':'');d.innerHTML='<div class="row"><b>'+esc(s.name||'حساب')+'</b><span class="badge">'+(s.connected?'🟢 متصل':'🔴 غير متصل')+'</span></div><div class="muted">@'+esc(s.username||'')+' · '+s.id+'</div>';d.onclick=()=>selectSession(s.id,d);$('sessions').appendChild(d)})}function filterSessions(){sessionFilter=$('sessionSearch').value;renderSessions()}async function selectSession(id,el){currentSession=id;currentChat=null;document.querySelectorAll('.item').forEach(x=>x.classList.remove('active'));el.classList.add('active');$('messages').innerHTML='<div class="empty">اختر محادثة.</div>';await loadDialogs()}async function loadDialogs(){if(!currentSession)return;const q=$('chatSearch').value||'';const ds=await api('/api/sessions/'+currentSession+'/dialogs?search='+encodeURIComponent(q));$('dialogs').innerHTML='';ds.forEach(d=>{const x=document.createElement('div');x.className='item';x.innerHTML='<div class="title">💬 '+esc(d.title)+'</div><div class="muted">'+esc(d.type)+' · '+(d.unread||0)+' غير مقروء</div>';x.onclick=()=>openChat(d.id,d.title);$('dialogs').appendChild(x)})}async function openChat(id,title){currentChat=id;$('chatTitle').textContent=title;$('chatSub').textContent='تحميل...';const ms=await api('/api/sessions/'+currentSession+'/chats/'+id+'/messages?limit=60');renderMessages(ms);$('chatSub').textContent=ms.length+' رسالة'}async function searchMessages(){if(!currentSession)return;if(currentChat){const q=$('chatSearch').value||'';const ms=await api('/api/sessions/'+currentSession+'/chats/'+currentChat+'/search?q='+encodeURIComponent(q));renderMessages(ms)}else await loadDialogs()}function renderMessages(ms){$('messages').innerHTML='';ms.slice().reverse().forEach(m=>{const x=document.createElement('div');x.className='msg '+(m.out?'out':'');let media='';if(m.media){const u=authUrl(m.media.url);if(m.media.kind==='image')media='<div class="media"><img src="'+esc(u)+'" loading="lazy"></div>';else if(m.media.kind==='video')media='<div class="media"><video src="'+esc(u)+'" controls preload="metadata"></video></div>';else media='<div class="media"><a class="file" href="'+esc(u)+'" target="_blank">📎 '+esc(m.media.name||'ملف')+'</a></div>'}x.innerHTML='<div>'+esc(m.text||'')+'</div>'+media+'<div class="meta">'+esc(m.date||'')+(m.sender?' · '+esc(m.sender):'')+'</div>';$('messages').appendChild(x)});$('messages').scrollTop=$('messages').scrollHeight}async function sendMessage(){if(!currentSession||!currentChat)return;const inp=$('message'),text=inp.value.trim();if(!text)return;await api('/api/sessions/'+currentSession+'/chats/'+currentChat+'/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});inp.value='';await openChat(currentChat,$('chatTitle').textContent)}init();
</script></body></html>"""
...
if FastAPI:
    ...
    async def _authorized_user(init_data: str = Query("")) -> int:
        if not init_data:
            raise HTTPException(401, "initData مطلوب. افتح Mini App من داخل Telegram.")
        try:
        ...
        now = int(time.time())
        if auth_date <= 0 or auth_date > now + 60 or now - auth_date > TELEGRAM_WEBAPP_INIT_DATA_MAX_AGE:
...
".join(f"{k}={v}" for k,v in sorted(pairs.items()))
    ...
    def _available_session_ids() -> list[int]:
        ...
        for path in SESSION_DIR.glob("user_*.session"):
            try: ids.add(int(path.stem.removeprefix("user_")))
            except ValueError: pass
        return sorted(x for x in ids if x>0)

    async def _web_client(sid:int, init_data:str=""):
        await _authorized_user(init_data)
        if sid not in _available_session_ids(): raise HTTPException(404,"جلسة Telegram غير موجودة")
        try:
            c=await get_or_create_client(sid)
            if not c.is_connected(): await c.connect()
            if not await c.is_user_authorized(): raise HTTPException(403,"الجلسة غير مسجل الدخول فيها")
            return c
        except HTTPException: raise
        except AuthKeyUnregisteredError as exc: raise HTTPException(403,"جلسة Telegram غير صالحة") from exc
        except Exception as exc: raise HTTPException(503,f"تعذر الاتصال بجلسة Telegram: {exc}") from exc

    @web_app.get("/", response_class=HTMLResponse)
    async def web_index(): return HTMLResponse(WEB_HTML)
    @web_app.get("/healthz")
    async def web_healthz(): return {"ok":True,"web":True,"mini_app":True,"port":WEB_PORT,"sessions":len(_available_session_ids())}
    @web_app.get("/api/sessions")
    async def web_sessions(init_data:str=Query("")):
        await _authorized_user(init_data); result=[]
        for sid in _available_session_ids():
            try:
                c=await _web_client(sid,init_data); me=await c.get_me(); name=f"{getattr(me,'first_name','') or ''} {getattr(me,'last_name','') or ''}".strip()
                result.append({"id":sid,"name":name or "حساب","username":getattr(me,'username','') or "","connected":c.is_connected()})
            except Exception as exc: logger.warning("تعذر فتح جلسة Mini App %s: %s",sid,exc)
        return result
    @web_app.get("/api/sessions/{sid}/dialogs")
    async def web_dialogs(sid:int,search:str="",init_data:str=Query("")):
        c=await _web_client(sid,init_data); out=[]
        async for d in c.iter_dialogs(limit=None):
            title=d.name or str(d.id)
            if search and search.lower() not in title.lower(): continue
            ent=d.entity; typ="مجموعة" if getattr(ent,"megagroup",False) else ("قناة" if getattr(ent,"broadcast",False) else "محادثة")
            out.append({"id":d.id,"title":title,"type":typ,"unread":d.unread_count})
        return out
    async def _message_json(c,chat_id,m):
        media=None
        if m.media:
            name=getattr(m.file,"name",None) or "ملف"; mime=getattr(m.file,"mime_type",None) or ""; kind="image" if mime.startswith("image/") else ("video" if mime.startswith("video/") else "file")
            media={"kind":kind,"name":name,"url":f"/api/sessions/{c._web_sid}/media/{chat_id}/{m.id}"}
        return {"id":m.id,"text":m.text or "","out":bool(m.out),"date":m.date.isoformat() if m.date else "","sender":getattr(getattr(m,"sender",None),"first_name",None),"media":media}
    ...
    async def web_messages(sid:int,chat_id:int,limit:int=60,init_data:str=Query("")):
        c=await _web_client(sid,init_data); c._web_sid=sid; msgs=[m async for m in c.iter_messages(chat_id,limit=min(max(limit,1),100))]; return [await _message_json(c,chat_id,m) for m in msgs]
    @web_app.get("/api/sessions/{sid}/chats/{chat_id}/search")
    async def web_search(sid:int,chat_id:int,q:str=Query(""),init_data:str=Query("")):
        c=await _web_client(sid,init_data); c._web_sid=sid; msgs=[m async for m in c.iter_messages(chat_id,limit=100,search=q)]; return [await _message_json(c,chat_id,m) for m in msgs]
    @web_app.get("/api/sessions/{sid}/media/{chat_id}/{msg_id}")
    async def web_media(sid:int,chat_id:int,msg_id:int,init_data:str=Query("")):
        c=await _web_client(sid,init_data); m=await c.get_messages(chat_id,ids=msg_id)
        if not m or not m.media: raise HTTPException(404,"الوسائط غير موجودة")
        async def gen():
            import io
            bio=io.BytesIO(); await c.download_media(m.media,file=bio); bio.seek(0)
            while True:
                chunk=bio.read(1024*1024)
                if not chunk: break
                yield chunk
        return StreamingResponse(gen(),media_type=getattr(getattr(m,"file",None),"mime_type",None) or "application/octet-stream")
    @web_app.post("/api/sessions/{sid}/chats/{chat_id}/send")
    async def web_send(sid:int,chat_id:int,body:SendBody,init_data:str=Query("")):
        c=await _web_client(sid,init_data); text=body.text.strip()
        if not text: raise HTTPException(400,"الرسالة فارغة")
        m=await c.send_message(chat_id,text); return {"ok":True,"id":m.id}

    web_server=None; web_server_task=None
    async def start_web_server():
        global web_server,web_server_task
        if web_server_task and not web_server_task.done(): return
        config=uvicorn.Config(web_app,host=WEB_HOST,port=WEB_PORT,log_level="info",access_log=True)
        web_server=uvicorn.Server(config); web_server_task=asyncio.create_task(web_server.serve())
        await asyncio.sleep(0.2)
        logger.info("WEB_READY %s",WEB_PUBLIC_BASE_URL)
else:
    web_app=None
    async def start_web_server(): return
...
def owner_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🌐 فتح Mini App", web_app=WebAppInfo(url=WEB_PUBLIC_BASE_URL))],
        [InlineKeyboardButton("👥 المستخدمون والجلسات", callback_data="admin:users")],
        [InlineKeyboardButton("🔁 نقل القنوات", callback_data="admin:transfers")],
...
async def post_init(application: Application) -> None:
    try:
        ...
        logger.exception("تعذر تشغيل Mini App Web Server")
    try:
